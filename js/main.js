// === main.js (popup Nómina + fix Guardar + guard anti‑doble carga) ===
if (window.__APP_LOADED__) {
  ...
  // ==========================
  // PIN básico (mínimo necesario aquí)
  // ==========================
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const isInteractive = (el) => { if (!el) return false; if (el.closest(filtrosSel) || el.closest(footerSel)) return true; return !!el.closest('button, a, select, input, textarea, label, [role="button"], [tabindex]'); };
    const applyUIVisibility = () => { const filtros = document.querySelector(filtrosSel); const footer  = document.querySelector(footerSel); if (filtros) filtros.style.display = fullscreenMode ? 'none' : ''; if (footer)  footer.style.display  = fullscreenMode ? 'none' : ''; };
    const toggleFullscreenUI = () => { fullscreenMode = !fullscreenMode; applyUIVisibility(); requestAnimationFrame(() => mostrar()); try { sessionStorage.setItem('ui_fullscreen', fullscreenMode ? '1' : '0'); } catch {} };
    try { const prev = sessionStorage.getItem('ui_fullscreen'); if (prev === '1') { fullscreenMode = true; applyUIVisibility(); } } catch {}
    window.addEventListener('touchstart', (ev) => { const t = Date.now(); const target = ev.target; if (isInteractive(target)) return; if (t - _lastTap <= TAP_WINDOW) { ev.preventDefault(); toggleFullscreenUI(); _lastTap = 0; } else { _lastTap = t; } }, { passive: false });
    ...
    // 🔒 Enlace seguro de formulario: evitar submit tradicional y asegurar botón guardar
    const form = document.getElementById('form');
    if (form && !form.getAttribute('data-bind')) {
      form.addEventListener('submit', (e) => { e.preventDefault(); guardar(); });
      form.setAttribute('data-bind','1');
    }
    const btnGuardar = document.getElementById('btnGuardar');
    if (btnGuardar && !btnGuardar.onclick) btnGuardar.onclick = guardar;
  });
  ...
  function mostrar() {
    ...
    if (modo === "graficos") {
      ...
      if (btnRight){ btnRight.innerHTML = ""; btnRight.onclick = null; btnRight.style.display = "none"; btnRight.style.position = ""; btnRight.style.left = ""; btnRight.style.top = ""; btnRight.style.transform = ""; btnRight.style.opacity = "0"; }
  ...
  const abrirFormulario = (id = null) => {
    ...
    const fechaEl = document.getElementById("fecha");
    if (fechaEl) { fechaEl.onchange = () => { if (selOrigen.value === "Nómina") { const mesPrefer = mesFromISO(fechaEl.value); const catActual = (document.getElementById("categoria") || {}).value || ""; onOrigenChange("Nómina", { preCat: catActual, preSub: mesPrefer }); } }; }
  ...
  // ==========================
  function normalizarListasExistentes(){ const vistosCat = new Set(Object.values(catBase).map(v => canonicalizeLabel(v))); const nuevaExtra = []; const unicosExtra = [...new Set(catExtra)]; for (let v of unicosExtra){ const k = canonicalizeLabel(v); if (vistosCat.has(k)) continue; if (NOMINA_CATS.map(canonicalizeLabel).indexOf(k) >= 0) continue; if (!nuevaExtra.some(x => canonicalizeLabel(x)===k)) nuevaExtra.push(v); vistosCat.add(k); } catExtra = nuevaExtra; localStorage.setItem('categoriaExtra', JSON.stringify(catExtra)); const vistosSub = new Set(); const nuevasSubs = []; for (let v of subMaestra){ const k = canonicalizeLabel(v); if (!vistosSub.has(k)) { vistosSub.add(k); nuevasSubs.push(v); } } subMaestra = nuevasSubs; localStorage.setItem('subMaestra_v2', JSON.stringify(subMaestra)); const catIndexCanon = buildCanonIndex([...catBase, ...catExtra, ...NOMINA_CATS], []); const subIndexCanon = buildCanonIndex([...subMaestra, ...NOMINA_SUBS], []); let cambiado = false; movimientos = movimientos.map(m=>{ const kc = canonicalizeLabel(m.c); const ks = canonicalizeLabel(m.s); let c = m.c, s = m.s; if (catIndexCanon.has(kc)) c = catIndexCanon.get(kc); if (subIndexCanon.has(ks)) s = subIndexCanon.get(ks); if (c!==m.c || s!==m.s){ cambiado = true; return {...m, c, s, ts: Math.max(Date.now(), (m.ts||0)+1)}; } return m; }).sort((a,b)=>new Date(b.f)-new Date(a.f)); if (cambiado) localStorage.setItem('movimientos', JSON.stringify(movimientos)); }
